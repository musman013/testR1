package com.nfinity.demo.petsdemotest.emailbuilder.emailconverter.utils;

import static com.nfinity.demo.petsdemotest.emailbuilder.emailconverter.utils.CommonUtil.checkValue;
...
import com.nfinity.demo.petsdemotest.emailbuilder.emailconverter.dto.request.Background;
import com.nfinity.demo.petsdemotest.emailbuilder.emailconverter.dto.request.Border;
import com.nfinity.demo.petsdemotest.emailbuilder.emailconverter.dto.request.Elements;
import com.nfinity.demo.petsdemotest.emailbuilder.emailconverter.dto.request.Height;
import com.nfinity.demo.petsdemotest.emailbuilder.emailconverter.dto.request.LineHeight;
import com.nfinity.demo.petsdemotest.emailbuilder.emailconverter.dto.request.Options;
import com.nfinity.demo.petsdemotest.emailbuilder.emailconverter.dto.request.Padding;
import com.nfinity.demo.petsdemotest.emailbuilder.emailconverter.dto.request.Request;
import com.nfinity.demo.petsdemotest.emailbuilder.emailconverter.dto.request.Size;
import com.nfinity.demo.petsdemotest.emailbuilder.emailconverter.dto.request.Structures;
import com.nfinity.demo.petsdemotest.emailbuilder.emailconverter.dto.request.Width;
...
public class EmailMjmlTemplateGenrator {
	...
	private String createPadding(Padding padding) {
		return checkValue.apply(padding.getTop(), "10") + "px " + checkValue.apply(padding.getRight(), "25") + "px "
				+ checkValue.apply(padding.getBottom(), "10") + "px " + checkValue.apply(padding.getLeft(), "25")
				+ "px";
	}

	private String createBackground(Background background) {
		return checkValue.apply(background.getColor(), "white") + " " + checkValue.apply(background.getUrl(), "")
				+ " && " + checkValue.apply(background.getUrl(), "") + " "
				+ checkValue.apply(background.getRepeat(), "no-repeat") + " top center";
	}

	private String createWidth(Width width) {
		String value = width.isAuto() ? "auto " : "";
		value = Arrays.asList("%", "px").stream().anyMatch(string -> string.compareTo(width.getUnit()) == 0)
				? value + width.getValue() + width.getUnit() : value;
	...
	private String createHeight(Height height) {
		...
		value = Arrays.asList("%", "px").stream().anyMatch(string -> string.compareTo(height.getUnit()) == 0)
				? value + height.getValue() + height.getUnit() : value;
	...
	private String createWidthHeight(Size size) {
		...
		value = Arrays.asList("%", "px").stream().anyMatch(string -> string.compareTo(size.getUnit()) == 0)
				? value + size.getValue() + size.getUnit() : value;
		return value;
	}

	private String createBorder(Border border) {
		return checkValue.apply(border.getWidth(), "4") + "px " + checkValue.apply(border.getStyle(), "solid") + " "
	...
	private String createLineHeight(LineHeight lineHeight) {
		return lineHeight.getUnit().compareToIgnoreCase("none") != 0
				? checkValue.apply(lineHeight.getValue(), "22") + checkValue.apply(lineHeight.getUnit(), "px") : "120%";
	}
	...
	private String getText(String innerText, Options options) {
		String template = "<mj-text \n css-class=\"ip-text-block\"" + "\n color=\"%1$s\"" + "\n font-family=\"%2$s\""
				+ "\n font-size=\"%3$spx\"" + "\n font-style=\"%4$s\"" + "\n font-weight=\"%5$s\""
				+ "\n text-decoration=\"none\"" + "\n align=\"left\"" + "\n line-height=\"%6$s\""
				+ "\n padding=\"%7$s\">" + "\n  %8$s" + "</mj-text>";

		template = String.format(template, options.getColor(), options.getFont().getFamily(),
				options.getFont().getSize(), options.getFont().getStyle(), options.getFont().getWeight(),
				this.createLineHeight(options.getLineHeight()), this.createPadding(options.getPadding()), innerText);
		return template;
	}
	...
	private String getImage(String src, Options options) {
		...
		template = String.format(template, this.createPadding(options.getPadding()),
				...
				options.getLink().getHref(), options.getLink().getTarget(), options.getAlign(), options.getTitle(),
				options.getTitle(), src);
		return template;
	}

	private String getButton(String innerText, Options options) {
		String template = "<mj-button" + "\n css-class=\"ip-button-block\"" + "\n background-color=\"%1$s\""
		...
		template = String.format(template, options.getBackground().getColor(), this.createBorder(options.getBorder()),
				...
				this.createLineHeight(options.getLineHeight()), options.getLink().getHref(),
				options.getLink().getTarget(), this.createPadding(options.getPadding()),
				this.createPadding(options.getPadding()), options.getFont().getFamily(), options.getFont().getSize(),
				options.getFont().getStyle(), options.getFont().getWeight(), innerText);
		return template;
	}

	private String getDivider(Options options) {
		String template = "<mj-divider" + "\n css-class=\"ip-divider-block\"" + "\n border-color=\"%1$s\""
				+ "\n border-style=\"%2$s\"" + "\n border-width=\"%3$s\"px" + "\n padding=\"%4$s\">" + "</mj-divider>";
		template = String.format(template, options.getBorder().getColor(), options.getBorder().getStyle(),
				options.getBorder().getWidth(), this.createPadding(options.getPadding()));
		return template;
	}

	private String getSpacer(Options options) {
		String template = "<mj-spacer" + "\n css-class=\"ip-spacer-block\"" + "\n padding=\"0\"" + "\n height=\"%1$s\">"
				+ "</mj-spacer>";
	...
	private String genrateTemplateSection(Structures structures) throws IOException {
		...
		String backgroundSize = !structures.getOptions().getBackground().getSize().getValue().isEmpty()
				? this.createWidthHeight(structures.getOptions().getBackground().getSize()) : "auto";
		String mjColumnString = "<mj-column  %1$s padding=\"0\"\n" + " vertical-align=\"top\" \n"
				+ " css-class=\"ip-column\"> ";
		Elements elements[][] = structures.getElements();
		String mjColumn = "";
		for (int i = 0; i < elements.length; i++) {
			Elements elements2[] = elements[i];
			mjColumn = mjColumn + String.format(mjColumnString,
					(Arrays.asList("cols_12", "cols_21").stream()
		...
		String finalTemplate = String.format(template, type, optionsBorder, optionsPadding, backgroundColor,
				backgroundUrl, backgroundRepeat, backgroundSize, elementsString);
	...
	public String genrateTemplate(Request request) throws IOException {
		...
		String structures = request.getStructures().stream().map(structure -> { // System.out.println(structure.getType());
			try {
		...
		String templatae = String.format(this.genrateTemplateBody(), hederPadding, hederDirection, hederBodyPadding,
				hederBodyBackground, headerBackgroundSize, bodyWidth, bodyBackgroundColor, structures);
