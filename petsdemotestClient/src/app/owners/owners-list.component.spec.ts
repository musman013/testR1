...
import { IOwners, OwnersService, OwnersDetailsComponent, OwnersListComponent, OwnersNewComponent } from './index';

describe('OwnersListComponent', () => {
  let fixture:ComponentFixture<OwnersListComponent>;
  let component:OwnersListComponent;
  let el: HTMLElement;
  let constData:IOwners[] = [
    {   
      address: 'address1',
      city: 'city1',
  ...
  describe('Integration tests', () => {
    ...
    it('should verify that redirected to details page when edit button is clicked', async () => {
      ...
      expect(location.path()).toBe(`/owners/${ServiceUtils.encodeIdByObject(data[0], component.primaryKeys)}`);
    });
