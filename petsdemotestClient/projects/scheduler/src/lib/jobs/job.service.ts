...
import { ISearchField, ServiceUtils } from 'projects/fast-code-core/src/public_api';;
...
export class JobService {
  ...
  constructor(private httpclient: HttpClient, @Inject(IP_CONFIG) private _config: IForRootConf) {
    this.baseUrl = _config.apiPath + "/jobs";
  }

  public getJobExecutionHistory(searchFields: ISearchField[], offset:number ,limit:number , sort:string): Observable<IExecutionHistory[]> {
    let params = ServiceUtils.buildQueryData(searchFields,offset,limit,sort);
    return this.httpclient.get<IExecutionHistory[]>(this.baseUrl + '/jobExecutionHistory',{params}).pipe(catchError(this.handleError))
  }
  ...
  public getAll(searchFields: ISearchField[], offset:number, limit:number, sort:string): Observable<IJob[]> {
    let params = ServiceUtils.buildQueryData(searchFields,offset,limit,sort);
    return this.httpclient.get<IJob[]>(this.baseUrl, {params}).pipe(catchError(this.handleError));
  }
  ...
  public create(item: IJob): Observable<IJob> {
    return this.httpclient
      .post<IJob>(this.baseUrl, item).pipe(catchError(this.handleError));

  }
  public update(item: IJob, jobName, jobGroup): Observable<IJob> {
    return this.httpclient
      .put<IJob>(this.baseUrl + '/' + jobName + '/' + jobGroup, item).pipe(catchError(this.handleError));
  }
  public delete(jobName, jobGroup): Observable<null> {
    return this.httpclient
      .delete(this.baseUrl + '/' + jobName + '/' + jobGroup).pipe(map(res => null), catchError(this.handleError));
  }
  ...
  public pauseJob(jobName, jobGroup): Observable<boolean> {
    return this.httpclient.get<boolean>(this.baseUrl + "/pauseJob/" + jobName + '/' + jobGroup).pipe(catchError(this.handleError));
  }

  public resumeJob(jobName, jobGroup): Observable<boolean> {
    return this.httpclient.get<boolean>(this.baseUrl + "/resumeJob/" + jobName + '/' + jobGroup).pipe(catchError(this.handleError));
  }
